...
class Task_old {
	static 	public function func($ADT,$TSK,$modnev)
	{	
		$func='';
		$task=$ADT::$task;
		if(isset($ADT::$resfunc) && method_exists($modnev,$ADT::$resfunc)){$func=$ADT::$resfunc;}
	...
	static 	public function next($ADT,$TSK)
	{	$task=$ADT::$task;
		if(isset($TSK::${$task}['next']) ){	$task=$TSK::${$task}['next'];
				}else{$task='';}
	...
	static 	public function modnev($ADT)
	{	
		$task=$ADT::$task;
		if(isset($ADT::$modnev) && $ADT::$modnev!=''){$modnev=$ADT::$modnev.$task;}
			else{$modnev='app';}//ha applikációban futtatjuk nem tud több példány létezni
...
static 	public function res($ADT,$TSK)
	{
		...
		while ($task!='') {
			...
			$trt=$TSK::${$task}['trt'];
			$modnev=self::modnev($ADT);
			
			if(!class_exists($modnev, false)){eval(Ob_Trt::str($modnev,$trt));}
		
			eval('$'.$modnev.'=new '.$modnev.'();');
			$func=self::func($ADT, $TSK, $modnev);
			if($func=='')
				{$task='';}
			else{
				eval('$ADT=$'.$modnev.'->'.$func.'($ADT,$TSK);');
				$task=self::next($ADT, $TSK);	
			}
		$ADT::$task=$task;
		$ADT::$next='';
		}
