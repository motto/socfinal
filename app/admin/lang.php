...
class MODPAR
{
    ...
    //admin tábla----------------------------
    public static $dataT = array();
    public static $tab_paramT  = [];
    public static $tab_szerkT  = [];

}
MODPAR::$tab_szerkT['id']        =['ell'=>'nosave'];
MODPAR::$tab_szerkT['app']       =['cim'=>'App','conv'=>''];
MODPAR::$tab_szerkT['label']     =[];
MODPAR::$tab_szerkT['value']     =[];
MODPAR::$tab_szerkT['megjegyzes']=[];
MODPAR::$tab_szerkT['lang']      =[];
MODPAR::$tab_szerkT['verse']     =[];
ADT::$LT['app']='App';
ADT::$LT['label']='Label';
class PAR
{
...
class View
{
    ...
    public static function szerk()
    {
        ...
        $textmezo=self::textmezo_lista();
        ADT::$view=str_replace('<!--|langmezok|-->',$textmezo,ADT::$view );
        $checkbox=self::checknox_lista();
        ADT::$view=str_replace('<!--|langcheckbox|-->',$checkbox,ADT::$view );
    }
    public static function checknox_lista()
    {
    $res='';
    $langT=PAR::$langT;
    if(isset($_POST['langT'])){$langT=$_POST['langT'];}
       // print_r($langT);
    foreach(PAR::$langT as $lang)
    {   $checked='';

        if(in_array($lang,$langT)){$checked='checked';}
        $mezo = <<<html
               <input type="checkbox" name="langT[]" value="$lang" $checked>$lang
html;
    ...
    public static function textmezo_lista()
    {
        $res='';
        $langT=PAR::$langT;
        if(isset($_POST['langT'])){$langT=$_POST['langT'];}
        foreach($langT as $lang)
        {
            $mezo = <<<html
                     <div class="form-group">
                         <label >$lang</label>
                         <textarea name="value[$lang]" class="form-control" rows="3">
                         <!--#value--> </textarea>
                    </div>
html;
 ...
 use joghiba;
    ...
    public function alap()
    {
     ...
     //   $ikonsor=MOD::ikonsor(PAR::$ikonsorT);
      ...
      //  ADT::$view=str_replace('<!--|tabla|-->',$admin_tab,ADT::$view );
